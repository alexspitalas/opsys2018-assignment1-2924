...
NUM=$( grep -v '^ *#' $1 | grep -c '^http*' ) # in NUM i hold the number of http files
touch htmlstemp # i copy all the http sites to file htmlstemp
grep -v '^ *#' $HFILE | grep '^http*'  > htmlstemp

for ((c=1; c<=$NUM;c++))
do
    htmlfun & # and here i check for the files in parallel
done
wait
rm htmlstemp
